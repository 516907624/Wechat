<template lang="pug">
    .box
      .header
        span <
        span web7班({{$parent.$data.mynamedata}})
        span
      .content
        .text(v-for='(userfixd,index ) in msg')
          .peopleleft(v-if='userfixd.usersnamedata !=$parent.$data.NewData.mytitle')
      ...
      methods:{
        Sendout(){
          if(this.mytext.length != 0 ){
            this.$socket.emit( 'usertext',this.mytext,this.$parent.$data.NewData.mytitle)
            this.mytext=''
          }else{
            $.toast('输入不能为空')
          }
